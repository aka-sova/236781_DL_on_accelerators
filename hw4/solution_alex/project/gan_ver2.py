...
class SN_Discriminator(nn.Module):
    def __init__(self, in_size):
        ...
        """
        # first layer
        modules.append(SN(nn.Conv2d(in_channels,out_channels=n,kernel_size=3,stride=1, padding=1)))
        #modules.append(nn.BatchNorm2d(n))
        modules.append(nn.MaxPool2d(2))
        modules.append(nn.ReLU())
        # second layer
        modules.append(SN(nn.Conv2d(in_channels=n,out_channels=n*2,kernel_size=3,stride=1, padding=1)))
        #modules.append(nn.BatchNorm2d(n*2))
        modules.append(nn.MaxPool2d(4))
        modules.append(nn.ReLU())
        # third layer
        modules.append(SN(nn.Conv2d(in_channels=n*2,out_channels=n*4,kernel_size=3,stride=1, padding=1)))
        #modules.append(nn.BatchNorm2d(n*4))
        modules.append(nn.MaxPool2d(8))
        modules.append(nn.ReLU())
        # fourth layer
        modules.append(SN(nn.Conv2d(in_channels=n*4,out_channels=n*8,kernel_size=1)))
        modules.append(nn.ReLU())
        # output layer
        modules.append(SN(nn.Conv2d(in_channels=n*8,out_channels=1,kernel_size=1)))
        modules.append(nn.Sigmoid())
        self.cnn = nn.Sequential((*modules))
...
def Wgen_loss_fn(y_generated):
    """
    Wasserstein loss function for Generator
    """
    # ====== YOUR CODE: ======
